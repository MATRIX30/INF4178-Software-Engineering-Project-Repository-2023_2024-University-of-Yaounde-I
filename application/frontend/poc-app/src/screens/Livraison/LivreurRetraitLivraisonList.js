...
import { LIVREUR_ID, SKELETON_LIST_LIMIT } from '../../constants/GLOBAL_VARIABLES'
import useAuthUser from '../../hooks/useAuthUser'
import useDataFetching from '../../hooks/useDataFetching'

const LivreurRetraitLivraisonList = ({route}) => {
    ...
    const [loadingD, messageD, deliveries, fetchDeliveries] = useDataFetching(
        (auth?.id && livreur) ? 'livreurs/' + livreur?.id + '/pocs/' + auth?.id + '/deliveries/accepted?page=0&size=20' : null,
    );
    ...
    useEffect(() => {
        navigation.setOptions({ title: (livreur?.firstName+" "+livreur?.lastName).toUpperCase() })
    }, [navigation]);
    return (
        <View style={{ backgroundColor: 'white' }}>
            <View style={{ backgroundColor: COLORS.primary, minHeight: "20%", marginBottom: '5%' }}>
                ...
                <View style={{ backgroundColor: COLORS.primary, alignItems: 'center', justifyContent: 'center', marginTop: '3%' }}>
                    <Image source={require('../../../assets/images/scarlett.png')} style={{ height: 70, width: 70, borderRadius: 100 }} />
                    <Text style={{ fontWeight: 'bold', marginTop: 5 }}>Code:{livreur?.code}</Text>
                </View>
                <View style={styles.infosPaquet}>
                    <View style={styles.infoContain}>
                        <Title color={COLORS.secondary} style={{ marginEnd: 5 }} size={SIZES.body} text={deliveries?.numberOfElements} />
                        <Title color={COLORS.black} size={SIZES.body} text={"Paquêts à retirer"} />
                    </View>
                </View>
            </View>
...
export default LivreurRetraitLivraisonList
